...
def affine_forward(x, w, b):
  ...
  x_ = np.reshape(x, newshape=(x.shape[0], -1))
...
def batchnorm_backward(dout, cache):
  ...
  x, mu, var, N, eps, gamma, x_hat = cache
  # calculating dx
  inv_var = 1/np.sqrt(var + eps) 
  dx_hat = dout*gamma
  prt1 = inv_var*dx_hat
  dl_dvar = -0.5*inv_var**3*np.sum((x-mu)*dx_hat, axis=0)
  prt2 = 2*(x-mu)/N*dl_dvar
  dl_du = inv_var*np.sum(dx_hat, axis=0) + 2/N*np.sum(x-mu, axis=0)*dl_dvar
  prt3 = -1/N*dl_du
  dx = prt1 + prt2 + prt3
