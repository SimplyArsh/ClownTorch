...
@dataclass
class D_data():
    def __init__(self, h_data=None, dup=None, shape=None):
        ...
        if h_data is not None:
            ...
            self.nbytes = int(np.dtype(np.float32).itemsize*self.flat_shape[0]*self.flat_shape[1])
            d_data = cuda.mem_alloc(self.nbytes) # d_W holds the reference to W on the GPU
            self.data = d_data    
    ...
    def to_host(self):
        h_out = np.zeros(shape=self.shape).astype(np.float32)
